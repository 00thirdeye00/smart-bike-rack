		...
		uint16_t datalen)
{
	...
	printf("LQI  from received packet:{{%u}}\n", aux);
	printf("accl_state rx            :{{%d}}\n", (int)data_rx.state_accl);
	printf("dist_state rx            :{{%d}}\n", (int)data_rx.state_dist);
	printf("rx at time:{{%lu}} and packet timestamp:{{%lu}}\n\n",(unsigned long)local_time_clock_ticks, (unsigned long)(data_rx.timestamp) );
}
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(obs_process, ev, data) {
	...
	is_coordinator = (node_id == 1);
        ...
        printf("is_coordinator: %d \n node_id %d \n", is_coordinator, node_id);
	if(is_coordinator){
